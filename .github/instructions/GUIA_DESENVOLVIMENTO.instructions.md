...
-    Para componentes de formulário:
     ...
     -    Mantenha a lógica no hook customizado local
...
## Convenções para Tratamento de Operações Assíncronas

-    Use o padrão do AlertContext para feedback:
...
1. ❌ **Instância Duplicada de Hook:**

     ```tsx
     // NO FILHO - NÃO FAÇA ISSO
     const MinhaFeatureForm = () => {
          const hook = useMinhaFeature(); // DUPLICADO!
          // ...
     };
     ```

2. ❌ **Estado Local Paralelo:**

     ```tsx
     // NO FILHO - NÃO FAÇA ISSO
     const MinhaFeatureForm = (props) => {
          const [localState, setLocalState] = useState(''); // Estado paralelo!
          // ...
     };
     ```

3. ❌ **Chamadas de Serviço no Componente:**

     ```tsx
     // NO COMPONENTE - NÃO FAÇA ISSO
     const MinhaFeatureForm = (props) => {
          const handleClick = async () => {
               const response = await service.getData(); // Chamada API no componente!
          };
     };
     ```

4. ❌ **Funções Não-Memoizadas nos Dependências de useEffect:**

     ```tsx
     // NÃO FAÇA ISSO - Causa loop infinito ou renderizações desnecessárias
     const handleChange = () => {
          /* lógica */
     };

     useEffect(() => {
          // Este efeito será executado em cada renderização
     }, [handleChange]); // handleChange recriada a cada renderização
     ```

5. ❌ **Recriação de Objetos/Arrays em Renderização:**
     ```tsx
     // NÃO FAÇA ISSO - Causa re-renderizações em componentes filhos
     return (
          <ComponenteFilho
               options={['opcao1', 'opcao2']} // Array criado a cada renderização
               config={{ chave: valor }} // Objeto criado a cada renderização
          />
     );
     ```
...
1. **Documentação JSDoc Completa:**
     ...
     ```tsx
     /**
      * @name useMinhaFeature
      * @author Gabriel M Guimarães | gabrielmg7
      * @description Hook customizado para gerenciar X e Y
      * @returns {Object} Retorna estados e handlers para o formulário
      * @package caminho/relativo/arquivo
      */
     ```
...
3. **Estrutura de Estado no Hook:**

     ```tsx
     // Estado para objeto principal
     const [entidade, setEntidade] = useState<IEntidade>(initialEntidade);

     // Estados derivados para campos de formulário
     const [campo1, setCampo1] = useState('');
     const [campo2, setCampo2] = useState('');

     // Metadados e configurações
     const pageTitle = id ? 'Editar' : 'Criar';
     ```
...
4. **Otimização de Renderização:**
     ...
     const pageTitle = useMemo(() => (id ? 'Editar' : 'Criar'), [id]);

     // Memoize funções manipuladoras
...
5. **Prevenção de Atualizações Desnecessárias:**

     ```tsx
     // Prevenir atualizações de estado quando não há mudança real
     useEffect(() => {
          if (!loading) {
               const novosItens = calcularNovosItens();

               // Verificar se realmente houve mudança antes de atualizar o estado
               if (JSON.stringify(itens) !== JSON.stringify(novosItens)) {
                    setItens(novosItens);
               }
          }
     }, [dependencias]);
...
6. **Carregamento Paralelo de Dados:**
     ...
     useEffect(() => {
          const fetchData = async () => {
               setLoading(true);
               try {
                    // Carregar em paralelo ao invés de sequencialmente
                    const [resultado1, resultado2, resultado3] = await Promise.all([servico1.getDados(), servico2.getDados(), servico3.getDados()]);

                    setDados1(resultado1);
                    setDados2(resultado2);
                    setDados3(resultado3);
               } catch (erro) {
                    handleError(erro);
               } finally {
                    setLoading(false);
